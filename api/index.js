...
function getProducts(request, response) {
  ...
  let data = []
  const sqlOpdracht = db.prepare('SELECT * FROM products JOIN Kleur ON Kleur.id = products.id JOIN soortpatroon ON soortpatroon.id = products.id JOIN soort ON soort.id = soortpatroon.soort_id JOIN patroon ON patroon.id = soortpatroon.patroon_id ORDER BY products.id ASC')
...
function getProductById(request, response) {
  ...
  const sqlOpdracht = db.prepare('SELECT products.* , Kleur.* FROM products LEFT JOIN Kleur ON products.id = Kleur.id WHERE products.id = ?')
